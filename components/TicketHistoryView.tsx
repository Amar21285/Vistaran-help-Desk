...
const eventStyles = {
    ...
    assignment: { icon: 'fas fa-user-tag', color: 'bg-indigo-500', title: 'Assignment Change' },
    notes: { icon: 'fas fa-sticky-note', color: 'bg-slate-500', title: 'Notes Updated' },
    default: { icon: 'fas fa-info-circle', color: 'bg-slate-500', title: 'Update' },
};

const getHistoryEventDetails = (change: string) => {
    const lowerChange = change.toLowerCase();
    if (lowerChange.includes('status changed')) return eventStyles.status;
...
const TicketHistoryView: React.FC<TicketHistoryViewProps> = ({ history = [], users }) => {
    // Sort from oldest to newest for a chronological timeline view
