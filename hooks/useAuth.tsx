...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
        ...
        if (savedUserId) {
            ...
            if (loggedInUser) {
                ...
                if (impersonatedUserId && loggedInUser.role === Role.ADMIN) {
                    ...
                    setUser(targetUser || loggedInUser); // Fallback to admin if impersonated user not found
                } else {
  ...
  const login = useCallback((email: string, password?: string): LoginStatus => {
    ...
    localStorage.removeItem('vistaran-helpdesk-impersonatedUserId'); // Clean up on new login
